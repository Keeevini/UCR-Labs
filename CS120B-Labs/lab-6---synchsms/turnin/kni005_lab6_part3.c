 ...
 *	Exercise Description: [Incrementing and Decrementing]
 *
 *	I acknowledge all content contained herein, excluding template or example
 *	code, is my own original work.
 *
 *	Demo Link: https://drive.google.com/file/d/1FjsgTIlYDjX84RrYuMbF1m1c9uayZ_FL/view?usp=sharing
 */
...
enum Action_States { NONE, INC, DEC } Action_State;
...
void tick() {
	...
	switch (SM_State) { // Transitions
		...
		case PRESSED:
			if ( (tmpInput & 0x03) == 0)
				SM_State = IDLE;
			if ( (tmpInput & 0x03) == 1 || (tmpInput & 0x03) == 2 )
				SM_State = WAIT;
			if ( (tmpInput & 0x03) == 3)
				SM_State = DOUBLE;
			break;
